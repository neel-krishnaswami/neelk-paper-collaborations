...
\> \> \;\;$U^{d'}_X(v, \term{v}, \mu', \sigma') \implies \Build^{d'}_Y(f\;v, \term{f\;v}, \mu', \sigma')$ 
\Newline[0.5em]

$U^d_{X \shrink Y}(f, \term{f}, \mu, \sigma) = $ \Newline
\> \;\;\= $\forall d' > 2\cdot d, \betterstate{d'}{\mu'}{\mu}, \sigma' \supseteq \sigma, v, \term{v}.$ \Newline
\> \>\;\; $U^{d'}_X(v, \term{v}) \implies \Build^{d'}_Y(f\;v, \term{f\;v}, \mu', \sigma')$ 
\Newline[0.5em]

$U^d_{A^\omega}(vs, \term{v}, (\theta, R)\;\mathrm{as}\;\mu, \sigma) = $ \Newline
\> \;\; $V_R(\term{v}) = vs \land D_R(\term{v}) = \U \land \Delta_R(\term{v}) \subseteq \sigma$ 
\Newline[1em]

$\Build^d_X(v, \term{code}, (\theta, R)\;\mathrm{as}\;\mu, \sigma) = $ \Newline
\> \;\;\= $\setof{H(\theta) \land \satisfyext{\mu}{d}}$ \Newline
\> \> \term{code} \Newline
\> \> $\{(\term{a}, \_).\;$\=$\exists \betterstate{d}{(\theta', R')\;\mathrm{as}\;\mu'}{\mu}, u.\;H(\theta') \;\land$\Newline
\> \> \> $U^d_X(v, \term{a}, \mu', \sigma) \;\land$ \Newline
\> \> \> $\Update(u, \mu, \mu', \sigma) \;\land$ \Newline
\> \> \> $\Stable(\mu, \mu')\}$
\Newline[0.5em]
