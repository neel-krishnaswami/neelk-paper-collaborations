...
stream programs which do not have a sensible interpretation as a
reactive program. For example, consider the following simple example:

\begin{tabbingspec}
\term{profit : Prices^\omega \to Orders^\omega} \Newline
...
$\Stream^d_A(vs, \term{v}, \mu) = $ \Newline
...
\> \> $\ready{\theta}{S_R(\term{r})}{-} \implies \Ref^{d'}_A(\head{\tail{V^R_L(r)}}, \term{r}, \mu)$ \Newline[1em]
