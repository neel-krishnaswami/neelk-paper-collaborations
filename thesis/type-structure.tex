...
constructor that permits nontermination and higher-order state. Its
syntax is given in figure~\ref{lang-type-syntax}.
...
of value the reference will point to. It is important that we do
...
$\monad{\tau}$.  The monadic type $\monad{\tau}$ is interpreted as a
sort of double-negation: $(\interpmono{\tau}(K_+, K_-) \to K_-) \to
K_+$.  So the $K_+$ and $K_-$ arguments are revealed as the positive
and negative occurrences of the ``answer type'' of a continuation.
One thing worth pointing out is that the positive and negative
occurrences do \emph{not} trade places on the recursive call, because
it occurs on the left-hand-side of the left-hand-side of a function
space (which is hence a positive occurrence). 

The careful reader will have noticed that so far our definitions do
not mention state at all. What will ultimately happen is that we will
interpret continuations as functions from heaps to the two-point
Sierpinski lattice $O = \setof{\top, \bot}$. However, since heaps can
contain references to values of polymorphic type, and since we have
not yet defined how the semantics of polymorphic types work, we have
...
is not defined over all types; it is only permitted to range over
...
rules for all of these forms are given in
...
does not justify parametricic principles of reasoning.

Below, we collect the theorems describing the properties of this judgment,
and give their proofs in the following subsection. 
