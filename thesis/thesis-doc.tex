...
CPO_\bot$ has a solution to the equation $X = F(X)$. Moreover, there
is also a \emph{minimal} isomorphism solving this equation. 
\end{prop}

\textbf{Proof.} The existence of a solution follows from Scott's 
inverse limit construction, together with Smyth and Plotkin's 
characterization of such solution. We give the details below. 
...
\to C$ (called a \emph{projection}) with the properties that $e \circ
p = id_D$ and $p \circ e \sqsubseteq id_C$. Furthermore, any such
projection is unique.

\begin{lemma}{(Uniqueness of Projections)}
  If $e : C \to D$ is an embedding with projections $p, p' : D \to C$, then
$p = p'$. 
\end{lemma}

\begin{proof}
Suppose $p \not= p'$. Then, there is a $d \in D$ such
that $p(d) \not= p'(d)$. However, since $p$ and $p'$ are projections,
we have that $e \circ p = e \circ p' = id$. Therefore, we know that $e(p(d)) =
d = e(p'(d))$, which is a contradiction. 
\end{proof}

\ \\
\noindent Now, consider the empty one-point domain $\emptyset_\bot =
...
\begin{lemma}{(Embeddings and Projections)} Each $e_i$, $p_i$ form
an embedding/projection pair between $X_i$ and $X_{i+1}$. 
\end{lemma}

\begin{proof}
This proof follows by induction on $i$. 
\begin{itemize}
...
\item Case $i=n+1$: 
  ...
  $F(p_n \circ e_n, p_n \circ e_n) \sqsubseteq F(id, id)$. 
\end{itemize}
\end{proof}

\ \\

\noindent Now, we'll define the domain $X$ to be the domain with the underlying set:
\begin{displaymath}
X \equiv \left\{ x \in (\Pi n:\N.\; X_n) \;|\; \forall m:\N.\; x_m = p_m(x_{m+1}) \right\}
\end{displaymath}
with the ordering being the usual component-wise ordering. To be in
$CPO_\bot$, it needs a least element, which is just $\lambda
n:\N.\;\bot$.  This set $X$ is the limit of the chain of domains
$X_i$, and so we will need to give embeddings and projections between
it and each $X_i$. To do so, we'll define:
