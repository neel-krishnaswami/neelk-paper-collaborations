...
object graph --- all inbound and outbound links must match. As a
result, simple local reasoning principles such as the frame rule of
separation logic (which permit ignoring any heap outside the footprint
of a command) no longer suffice for modular verification. As a result,
it is difficult to verify different parts of a dependency network
separately and combine their correctness proofs.
...
We illustrate the utility of this style of proof with a case study
from functional reactive programming, showing how to prove rules which
justify reasoning about an extremely imperative implementation as if
it were pure.
\end{abstract}
...
such as functions, sums, and products. This simplifies reasoning even
about imperative programs, because we can relatively freely
...
interpretations are pointed CPOs in domain theory.}  Also, we will
...
construct, and because this form expects the term to be bound to be an
...
generate it, and the $\return$ and $\bind$ operations to propagate it
...
specifications say the simply read and update their argument
...
$\ctext{mark\_unready}$ function does
(line~\ref{notify:mark-unready}). Given a cell, it takes all of the
observers of the current cell and recursively makes all of them
unready. Then it removes the current cell from the observer sets of
...
reasoning about the behavior of $\codetype{A}$ expressions. 
...
\nextlinelabel[0.75em]{invariant:value-rel}
...
\nextlinelabel[0.75em]{invariant:heap}
$heap(D,h) = $ \nextline
\;\;$counter \pointsto |D| \;* $ \nextline
\;\;$\forall^* c \in D.\;$\=$\exists v_r, v_o : \cellset.\;$ \nextline
                         ...
                         \>$set(D, v_o, obs(h(c)))$
\nextlinelabel[0.75em]{invariant:localstate}
...
$a$ must not have a value. Finally, the clause for $\localref{r}{v}$ is simply
...
finds each local reference formula in $\phi$ and asserts it is in
the physical heap.
...
state $\phi$, evaluating the $\codetype{\tau}$ expression $e$ will result
in a modified state $\phi'$ and a return value $v$ of type $\tau$. The
expression $e$ will have directly read the cells in $r$, and will have
evaluated the cells in $u$, sending them from an unready to a ready state.'' 
...
be a notational abbreviation for a formula in our logic of
specifications.  

The step relation $\astep{\phi}{e}{\phi'}{v}{r}{u}$ really means four
...
Here, we present a number of implications over specifications in
...
network library in terms of its action on the abstract heap. The combination
of quantification and ramification give us a domain-specific frame
...
which is intended to be driven by an event loop. Finally, we will show
the proving the correctness of a realization relation between the
...
\nextlinelabel[0.75em]{transduce:impl:lift}
...
\nextlinelabel[0.75em]{transduce:impl:seq}
...
\nextlinelabel[0.75em]{transduce:impl:par}
...
\nextlinelabel[0.75em]{transduce:impl:switch}
...
\nextlinelabel[0.75em]{transduce:impl:loop}
...
input cell is updated, invalidations are propagated throughout the
dependency network, and when the outputs are read, exactly the
...
Figure~\ref{imperative-transducer-semantics} in informal terms. 
(In the definitions, we suppress the type arguments to functions
in order to make the code more readable.) Here,
we define the type of imperative stream transducers from types $\alpha$
to $\beta$ as the type $\celltype{\alpha} \to \monad{(\celltype{\beta})}$.
This type should be read as saying that the implementation is a function that, 
given an input cell of type
$A$, will \emph{construct} a dataflow notification network realizing
the corresponding transducer, and whose return value is the output
cell of type $B$ that the event loop should read.
...
equal to $k$. Notice that the demand-driven nature of evaluation means
...
relaxed, but for the basic FRP combinators we consider in this paper
there is no need. Doing so \emph{would} become necessary if we added
combinators that dynamically created new transducers as the program
ran, since at that point we would need to create new cells at each
time step.)
...
input, may require evaluating some auxilliary network state. 
...
Then, the conclusion of this implication over triples, the second
...
\begin{tabbing}
$\forall f:A\to B.\; Relate(\liftop f, \lifttrans\;f)$ \\[1em]

$\forall p, f, q, g.\;$\=$Relate(p, f) \specand Relate(q, g)$ \\
                       \>$\specimp Relate(\seqop p\;q, \seqtrans\;f\;g)$ \\[1em]

$\forall p, f, q, g.\;$\=$Relate(p, f) \specand Relate(q, g)$ \\
                       \>$\specimp Relate(\parop p\;q, \partrans\;f\;g)$ \\[1em]

$\forall k, p, f, q, g.\;$\=$Relate(p, f) \specand Relate(q, g)$ \\
                          \>$\specimp Relate(\switchop k\;p\;q, \switchtrans\;k\;f\;g)$ \\[1em]
...
and $\liftop (g \circ f)$ both realize the same function $\lifttrans\;(g\circ f)$. 
...
by \citet{tldi09,ftfjp07} and work by \citet{parkinson-iwaco-07}. 
In both of these works, the program
invariant explicitly specifies the observers listening to a particular
subject, and the invariants for each observer. For a chain of
observers only one level deep, this works reasonably well, but it
breaks down when there are chains of dependencies -- when we have an
object which both observes and is observed by yet others. This is
troublesome because (roughly) we need to specify the transitive
closure of the reachability graph of observers from any subject, which
makes it impossible to locally add an observer to a subject, since we
then need to modify the invariants of everything the subject listens
to, as well.
...
history invariants~\cite{liskov-wing} to model observers. The work by
\citet{shaner-leavens-naumann} is also non-modular, since it requires
knowing what all of the observers are. The work on history invariants
\emph{is} modular, but it sharply restricts the kinds of invariants 
that can be used, in ways that make it very difficult to model the 
...
incrementally recompute answers as the inputs are adjusted. It would
be worth studying if our techniques could be useful in verifying
...
\citet{afrp}, and secondly, we use a discrete model of time, rather
...
existentially quantified cells (i.e., terms of type $\exists
\alpha:\star.\; \celltype{\alpha}$). Specifying this interface is not
...
All of the usual set operations are supported. We have an
$\ctext{emptyset}$, representing an empty set of cells. We have
...
We impose three axioms that any implementation must satisfy. First, we
require that if a $\cellset$ value $v$ represents a set $elts$ in a
world $W$, it will also represent a set in any larger world $W'$ ---
that is, allocating new cells cannot disturb already-existing cells.
Second, the values in a set are always a subset of the world $W$.
Finally, we require that $set(W,v,elts)$ is a pure predicate (i.e., is
not heap-dependent). In other words, we ask that $\cellset$ have a
purely functional implementation (for example, as a binary tree). This
requirement is not a necessity, but was chosen to simplify the
statement of the other invariants in this paper.
