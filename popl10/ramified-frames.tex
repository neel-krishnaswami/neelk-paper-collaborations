...
between the implementation of $\updatecell$, and in $\ctext{read}$.
...
of the reads relation, $R^+$ form a strict partial order. Strictness
...
R_H$).  Next, we require that the reads relation $R_H$ is a subset of
...
For example, we can prove the soundness of the following triple specifying
$\updatecell$. 
...
that a positive formula must have everything in its read set to be
...
\begin{proof}
This is much than $\updatecell$: we just need to allocate a new
...
cells and send them an unready state in the precondition to a ready
state in the postcondition. However, we may have had some cell formulas in
our frame which claimed their corresponding cell were unready purely
...
unready, given that we know the cell heap satisfies $\phi$.
...
\begin{prop}{(FRP Combinator Lemmas)}
First, we define the $Relate$ predicate as follows:
\begin{tabbing}
...
\phi)$ looks a family of modal operators on the formula $\phi$, but
...
propagation to write programs that dynamically and incrementally
...
auxiliary predicates, $matches$ and $iterseq$, which assert that a
