...
\combine{(R_1, L_1)}{(R_2, L_2)} = 
  \left[\begin{array}{l}
           \mathrm{let}\; N = L_1 - R_2 \;\mathrm{in}\\ 
           \IfThenElse{N < 0}{(R_1 - N, L_2)}{(R_1, N + L_2)} \\
         \end{array}
  \right.
...
So a string of type $\zero$ is just a strictly well-bracketed string, whereas
one of type $(0, 1)$ will be split into two strictly well-bracketed pieces

\begin{lemma}{(Every String Has a Unique Type)}
For every string $s$, there is a unique $d$ such that $s$ is bracketed
up to $d$.
\end{lemma}
...
Now, by induction we know that $s'$ is uniquely bracketed by a type $(R, L)$. 
\end{proof}


Finally, we will say a language $L$ is well-bracketed up to $d$, if every
