...
\newcommand{\Interp}[1]{(\!|{#1}|\!)}
\newcommand{\derivhat}[2]{\hat{d}_{#1}({#2})}
\newcommand{\setof}[1]{\{{#1}\}}
...
\section{The Dioid Structure of Bases}

Now, we know that the height of a derivative is bounded, but we still
are not quite where we need to be. The reason is that a basis expression is 
a \emph{tree}, and not a \emph{list}. 

\begin{mathpar}
  \begin{array}{lcl}
    s & ::= & (g, g) \bnfalt s \otimes s' \bnfalt 0 \bnfalt s \oplus s'
  \end{array}
\end{mathpar}

\begin{mathpar}
  \begin{array}{lcl}
    \Interp{(g, g)}       & = & \setof{[g, g']} \\
    \Interp{s_1 \otimes s_2} & = & \comprehend{ {gs}_1 :: g :: {gs}_2 }
                                              { ({gs}_1 :: g) \in \Interp{s_1} \land
                                                (g :: {gs}_2) \in \Interp{s_2} } \\
    \Interp{0} & = & \emptyset \\
    \Interp{s_1 \oplus s_2} & = & \Interp{s_1} \cup \Interp{s_2} \\[1em]
...
\begin{lemma}{(Axioms)}
  ...
  \begin{enumerate}
    \item $s \oplus s' = s' \oplus s$
    \item $s \oplus s = s$
    \item $s \oplus 0 = s$
    \item $0 \otimes s = 0$
    \item $s \otimes 0 = 0$
    \item $(s \otimes s') \otimes s'' = s \otimes (s' \otimes s'')$
    \item $(s_1 \oplus s_2) \otimes s_3 = (s_1 \otimes s_3) \oplus (s_2 \otimes s_3)$
    \item $(g, g') \otimes (g'', g''') = 0$ when $g' \not= g''$
    \item $(g, \epsilon) \otimes (\epsilon, g') = (g, g')$
    \item $(\epsilon, g) = (g, \epsilon)$
  \end{enumerate}
\end{lemma}

The critical properties are the last four. They will enable us to take a
tree of height $k$ -- the basis expression -- and turn it into a list
of \emph{length} $k$. 

\begin{lemma}{(Undistribution)}
  We know that
  \begin{mathpar}
  [(g_1, g_2) \otimes (g_3, g_4)] \oplus [(g'_1, g'_2) \otimes (g'_3, g'_4)] = 
  [(g_1, g_2) \oplus (g'_1, g'_2)] \otimes [(g_3, g_4) \oplus (g'_3, g'_4)]
  \end{mathpar}
\end{lemma}

\begin{proof}
  Suppose $g_2 = g_3$ and $g'_2 = g'_3$. 
...
\begin{mathpar}
  \begin{array}{lcl}
    \emptify{(g_1, g_2)} & = & \emptify{g_1} \land \emptify{g_2} \\
    \emptify{s_1 \otimes s_2} & = & \emptify{s_1} \land \emptify{s_2} \\
    \emptify{0} & = & \false \\
    \emptify{s_1 \oplus s_2} & = & \emptify{s_1} \vee \emptify{s_2} \\
  \end{array}
  \\
  \begin{array}{lcl}
    \derivhat{\sigma}{(g_1, g_2)} 
    & = & \IfThenElse{\emptify{g_1}}
                     {(\deriv{\sigma}{g_1}, g_2) \oplus (\epsilon, \deriv{\sigma}{g_2})}
                     {(\deriv{\sigma}{g_1}, g_2)} 
    \\
    \derivhat{\sigma}{s_1 \otimes s_2} 
    & = & (\derivhat{\sigma}{s_1} \otimes s_2) \oplus 
          (\IfThenElse{\emptify{s_1}}{\derivhat{\sigma}{s_2}}{0}) 
    \\
    \derivhat{\sigma}{0} 
    & = &
    0
    \\
    \derivhat{\sigma}{s_1 \oplus s_2}
    & = & 
    \derivhat{\sigma}{s_1} \oplus \derivhat{\sigma}{s_2}
    \\
  \end{array}
\end{mathpar}
