...
recognizing a regular language are regular expressions which describe
the language recognizable from that state. In our case, this simple
intuition is not quite good enough. A balanced language cannot be
recognized with a finite state machine, since we need a stack to match
the open and closing parentheses. 

So, to capture this intuition, we use basis expressions, which are
trees of 0-balanced grammatical expressions, with each 0-balanced
grammatical expression separated by right-parentheses. The idea is
that each time we see a left-parenthesis, we can ``push a state on
the stack''. The tree structure arises from the fact that 
...
For any $\judgebalance{\Gamma}{g}{0}$ there is a finite set $G$ of
