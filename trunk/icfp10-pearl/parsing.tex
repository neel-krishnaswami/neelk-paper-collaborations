...
\item $\deriv{s}{g_1 \cdot g_2} \simeq \deriv{s}{g_1}\cdot g_2 \vee \bigvee_{\comprehend{s_1, s_2}{s = s_1\cdot s_2}} \emptify{\deriv{s_1}{g_1}}\cdot \deriv{s_2}{g_2}$
  \begin{caseblock}
    ...
    \begin{case}{$s = \sigma \cdot s'$}
      \begin{mathpar}
        \begin{array}{llcl}
          \mbox{Def.} & \deriv{\sigma\cdot s'}{g_1 \cdot g_2} & = & 
...
\begin{caseblock}
  ...
  \begin{case}{$g = \fix{x}{g'}$}
    By hypothesis, we have $\judgebalance{\fix{x}{g'}}{0}$, and a $\gamma$ for $\Gamma$. 

    By inversion, we know that $\judgebalance[\Gamma, x]{g'}{0}$. 

    Now, take $\gamma' = \gamma, [\gamma(\fix{x}{g})/x]$ as a substitution for $\Gamma, x$. 

    By induction, we have a set $G'$ such that for $\gamma'(g')$ is based in $G'$. 

    Take $G = G' \cup \setof{\gamma(\fix{x}{g'})}$. 

    Now, consider an arbitrary string $s$. If it is empty, then $\deriv{\epsilon}{g)} = g \in G$. 
    ...
    Since we know that $\deriv{\sigma}{\gamma'(g')}$ is derived from the basis $G$, we know that
    $\deriv{s'}{\deriv{\sigma}{\gamma'(g')}}$ is also derived from the basis $G$. 
  \end{case}

  \begin{case}{$\judgebalance{g_1\cdot g_2}{d}$}
    By inversion, we know that $\judgebalance{g_1}{d_1}$ and $\judgebalance{g_2}{d_2}$.
    
    Assume we have some grounding substitution $\gamma$. 
    ...
    $g_1$ and $g_2$ generated from $G_1$ and $G_2$. Hence $g'_1\cdot g'_2$ is derived
    from the basis $G_1 \cup G_2$. 
    ...
    Now, note that $\deriv{\sigma}{g_1}$ is based in $G_1$, $g_2$ is
    based in $G_2$, and $\deriv{\sigma}{g_2}$ is based in $G_2$. Hence
    the whole expression is based in $G_1 \cup G_2$.
  \end{case}

  \begin{case}{$\judgebalance{g'*}{0}$}
    By inversion we have $\judgebalance{g'}{0}$ and assume we have $\gamma$ grounding $\Gamma$. 

    By induction $\gamma(g')$ is based in $G'$. 
