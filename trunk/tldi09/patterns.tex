...
\usepackage{amsmath}
\usepackage{amssymb}
% \usepackage[left=4.8cm,right=4.8cm]{geometry}
\input{commands.tex}
...
\author{Neelakantan R. Krishnaswami\inst{1} \and Jonathan Aldrich\inst{1} 
...
% \vspace{-1em}
% {\large \textbf{TODO: Fix line numbering}}

% It is more difficult to reason about programs that use aliasing than
% programs that do not use mutable shared data. Likewise, it is more
% difficult to reason about programs that use higher order features than
% first order programs. When both features are combined, we run into new
% problems which are more complex than the sum of these
% already-challenging parts.
% 
% Furthermore, higher-order imperative programs are commonly found in
% practice. Object-oriented programs make heavy use of both shared
% state, and objects themselves can be seen as a form of higher-order
% state, via the closure-object correspondence. So beyond the inherent
% technical interest in learning how to reason about higher-order
% imperative programs, there are also great practical applications for
% good answers to the question of how to verify such programs.
...
\textbf{Acknowledgments.} This work was supported in part by NSF grant
