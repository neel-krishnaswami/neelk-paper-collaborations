...
This clause is interpreted as $\comp{\interps{A}}$, which can be
understood as follows. In the category of ultrametric spaces, we want
to understand streams as pure values. However, we want to represent
streams via imperative data structures which change over time. This
conflict is resolved by representing a stream as a thunk, which
constructs and initalizes a stream data structure each time it is
evaluated. So each time we evaluate a value of type $A^\omega$, we
construct a fresh stream, ready to begin counting regardless of when
it was created.
...
\label{ufunc}
$U^d_{X \To Y}(f, \term{f}) =  \forall d' > 2\cdot d, v, \term{v}.\; U^{d'}_X(v, \term{v}) \implies \UBuild^{d'}_Y(f\;v, \term{f\;v})$ 
\Newline[0.5em]
...
\label{ustream}
$U^d_{A^\omega}(vs, \term{v}) = \forall \mu \in \Mem{d}.\;\Build^d_{S(A)}(vs, \term{v}, \mu, \emptyset, \U)$ 
\Newline[0.5em]

\label{ubuild-start}
$\UBuild^d_X(v, \term{code}) = $ \Newline
\;\;\= $\forall (\theta,R)\;\mathrm{as}\;\mu \in \Mem{d}.\;$ \Newline
\> \;\;\= $\setof{H(\theta) \land \satisfyext{\mu}{d}}$ \Newline
\> \> \term{code} \Newline
\> \> $\{(\term{a}, \_).\;$\=$\exists \betterstate{d}{(\theta', R')\;\mathrm{as}\;\mu'}{\mu}, u.\;H(\theta') \land U^d_X(v, \term{a}) \;\land$\Newline
\> \> \> $\Update(u, \mu, \mu', \emptyset) \land \Stable(\mu, \mu')\}$
\label{ubuild-end}
\Newline[1em]
