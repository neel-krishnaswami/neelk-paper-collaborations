...
The last two clauses connect the mathematical structure and the
...
\oooo Likewise, we know that if $(c, o') \in O_H$, then $o' \in os$, and symmetrically for $R_H$. \\ 
...
\ooox and a physical state $\mathit{heap}(D, h|_{\setof{os}})$, which (using lemmas 78 and 79)  \\
\ooox satisfies the property $\WF((D,h|_{os}))$\\
...
\item For all $vs, I, O$, and $U$, if we have $\Stream(i, vs, I, O, U)$ then it follows that $\Stream(o, \mathit{switch}\;k\;f\;g\;vs, I \otimes (\phi\;vs), O \otimes (\phi'\;vs), U \cup U')$
  ...
  \\ \oo 6. We want for all $n$, that $\ready{O_n \otimes (\phi'\;vs)_n}{o}{(\mathit{loop}\;a_0\;f\;vs)_n}$ \\
