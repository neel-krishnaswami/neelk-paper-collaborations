...
only if $p \leq q \wand r$. 
  ...
  \begin{tabbedproof}
    ...
    \oooo So we know, $p \subseteq \comprehend{m}{\forall m' \in q. m \# m' \mbox{ and } m' \in q \mbox{ implies } (m \cdot m') \in r}$ \\
    \oooo Therefore $\forall m. m \in p \mbox{ implies } \forall m' \in q.\; m \# m' \mbox{ and } m' \in q \mbox{ implies } (m \cdot m') \in r$.  \\
    \oooo Therefore $\forall m, m'.\; m \in p \mbox{ and } m' \in q \mbox{ and } m \# m' \mbox{ implies } (m \cdot m') \in r$ \\
    \oooo Therefore $\forall m_o, m, m'.\; m \in p \mbox{ and } m' \in q \mbox{ and } m \# m' \mbox{ and } m_o = m\cdot m' \mbox{ implies } m_o \in r$ \\
    \oooo Therefore $\forall m_o, (\exists m, m'.\; m \in p \mbox{ and } m' \in q \mbox{ and } m \# m' \mbox{ and } m_o = m\cdot m') \mbox{ implies } m_o \in r$ \\
    \oooo Therefore $\forall m_o, m_o \in (p * q) \mbox{ implies } m_o \in r$ \\
    \oooo Therefore $p * q \subseteq r$ \\
    \oooo Therefore $p * q \leq r$ \\
...
Now we'll prove a minor lemma about this function, which shows that we
...
\item Case \textsc{CtxCons}: 
  \begin{tabbedproof}
    ...
    \oooo $\eta \in \interp{\restrictkind{\Delta} \vdash
                              \restricttype{\Delta}}\;
                      \restricttyenv{\Delta}{\delta}\;(K,K)$ and \\
    \oooo $U(\eta) = \restrictvals{\Delta}{\delta}$ \\
    ...
    \ooooo By weakening, $\interp{\restrictkind{\Delta} \vdash
                          ...
                          \interp{\restrictkind{\Delta}, u:\kappa \vdash
                              \restricttype{\Delta}}\;
                      (\restricttyenv{\Delta}{\delta}, x)$\\
    \ooooo By equality, $\eta \in \interp{\restrictkind{\Delta}, u:\kappa \vdash
                              \restricttype{\Delta}}\;
    ...
    \ooooo By equality, $\eta \in \interp{\restrictkind{\Delta}, u:\kappa \vdash
                       ...
                       \restricttyenv{\Delta, u:\kappa}{(\delta, x)}\;(K,K)$\\
    \ooooo Therefore $U(\eta) = \restricttyenv{\Delta, u:\kappa}{(\delta,x)}$ \\
    \oooo If $\omega = A$: \\
    ...
    \ooooo By equality, 
           $\interp{\restrictkind{\Delta} \vdash \restricttype{\Delta}}\;
            ...
            \restricttyenv{\Delta}{(\delta, x)}$ \\
    \ooooo So $\eta \in \interp{\restrictkind{\Delta, u:A} \vdash \restricttype{\Delta}}\;
    ...
    \ooooo Therefore, $(\eta, x) \in 
                         \interp{\restrictkind{\Delta, u:A} \vdash \restricttype{\Delta}}\;
            \restricttyenv{\Delta}{(\delta, x)} \times T$ \\
    \ooooo By semantics of contexts, $(\eta, x) \in 
           \interp{\restrictkind{\Delta, u:A} \vdash \restricttype{\Delta, u:A}}\;
                   \restricttyenv{\Delta}{(\delta, x)}$ \\
    \ooooo And from induction hypothesis and $x \in U(T)$, we know \\
    \ooooo $U(\eta, x) \in U(\interp{\restrictkind{\Delta, u:A} \vdash \restricttype{\Delta, u:A}}\;
                   \restricttyenv{\Delta}{(\delta, x)})$ \\
    \ooooo So the witness in this case is $(\eta,x)$ \\
    \oooo If $\omega \not= A$ and $\omega \not= \kappa$: \\
    ...
    \ooooo Then $\restricttyenv{\Delta, u:\omega}{(\delta, x)} = \restricttyenv{\Delta}{\delta}$ \\
    \ooooo Then by equalities, $\eta \in 
              \interp{\restrictkind{\Delta, u:\omega} \vdash
                      \restricttype{\Delta, u:\omega}}\;
                     \restricttyenv{\Delta, u:\omega}{(\delta,x)}\;(K,K)$ and \\
    \ooooo $U(\eta) =  \restrictvals{\Delta, u:\omega}{(\delta,x)}$ \\

  \end{tabbedproof}
\end{itemize}
\end{proof}

Since the $\eta$ constructed in this proof has the same values as the
restricted substitution, we will often abuse notation and write things 
like $\restrictvals{\Delta}{\delta}$ to indicate the environment in 
CPO. 
...
A specification $\judgeS{S}$ is \emph{valid} if and only if, for all
