  ...
  \begin{tabbedproof}
    ...
    \ooo Expanding definitions, 
          $\comprehend{m}{\exists m_1, m_2.\; m_1 \# m_2 \land m_1 \in p \land m_2 \in q 
                                             \land m_1 \cdot m_2 = m}
           \subseteq r$ \\
    \ooo By set properties, $\forall m.\; (\exists m_1, m_2.\; m_1 \# m_2 \land m_1 \in p \land m_2 \in q 
                                             \land m_1 \cdot m_2 = m) \implies m \in r$. \\
    \ooo By existential fiddling and instantiating $m$, \\
    \oooo
...
$r \leq \bigwedge P$ and $\forall p \in P.\; \bigwedge P \leq p$. 
  ...
  \begin{tabbedproof}
    ...
    \oooo Instantiating $p'$ with $p$, we get $m \in p$. \\
  \end{tabbedproof}

\item We want to show $\forall r, P \subseteq B$, if $(\forall p \in P.\; p \leq r)$, then
$\bigvee P \leq r$ and $\forall p \in P.\; p \leq \bigwedge P$. 
  ...
  \begin{tabbedproof}
    ...
    \ooo Second, we want to show $\forall p \in P.\; p \leq \bigwedge P$. \\
    \oooo Assume $p$, $p \in P$ \\
    \ooooo We want to show $p \leq \bigwedge P$ \\
    ...
    \oooo Therefore, $p \leq \bigwedge P$ \\
  \end{tabbedproof}
...
If $X \subseteq \upset{P}$, then $\bigvee X$ defines a meet. 
\end{lemma}
...
\noindent Next, we need to show that $\forall S \in \upset{P}, X \subseteq \upset{P}$, 
if $(\forall S' \in X.\; S' \leq S)$, then $\bigvee X \leq S$ and $\forall S' \in X.\; S' \leq \bigvee X$. 
...
\end{lemma}
\begin{proof}
First, we need to verify the join we defined actually maps into the 
appropriate domain. 
...
\oooo Assume $x \in \bigvee X$, $y \in P$, $x \sqsubseteq y$ \\
\ooooo Since $x \in \bigvee X$, we know $\exists S \in X.\; x \in S$ \\
\ooooo Let $S$ be the witness of the existential, so $S \in X$ and $x \in S$ \\ 
\oooooo  Since $S$ is upwards-closed, $x \in S$, and $x \sqsubseteq y$, we know $y \in S$ \\
\oooo Therefore we can conclude $\exists S \in X.\; y \in S$ \\
\oooo Therefore $y \in \bigvee X$ \\
\end{tabbedproof}

\noindent Now, we need to show that $\forall S \in \upset{P}$, if $X \subseteq
...
First, we'll check that the definition puts us in the right codomain. 

Suppose that $x \in S_1 \implies S_2$, and that $y \sqsupseteq x$.
...
\begin{displaymath}
  \basicspec{p}{c}{a:A}{q(a)} \triangleq
    \forall h \in p. c\;Approx(q)\;h = \bot 
\end{displaymath}
...
\oooo This is equivalent to $\forall h \in p. \bot\;Approx(q)\;h = \bot$ \\
\oooo Assume $h \in p$ \\
\ooooo  By definition of least element, $\bot\;Approx(q)\;h = \bot$ \\
\oooo Therefore $\forall h \in p. \bot\;Approx(q)\;h = \bot$ \\
...
\oooo This is equivalent to $\forall h \in p * s.\; (\sqcup c_i)\; Approx(\semfun{a}{q(a)*s})\;h = \bot$ \\
\oooo Assume $h \in p * s$ \\ 
\ooooo By continuity, we know $(\sqcup c_i)\; Approx(\semfun{a}{q(a)*s})\;h = 
                               \bigsqcup (c_i\;Approx(\semfun{a}{q(a)*s})\;h)$ \\
...
\oooooo This is equivalent to $\forall r, s \sqsupseteq r, h \in p*s, c\;Approx(\semfun{a}{q(a)*s})\;h = \bot$ \\
\oooooo Instantiating quantifiers with $r$, $s$, and $h$, we get $c\;Approx(\semfun{a}{q(a)*s})\;h = \bot$\\
\ooooo Therefore $\forall c \in \comprehend{c_i}{i \in \N}$, $c\;Approx(\semfun{a}{q(a)*s})\;h = \bot$ \\
\ooooo Therefore $\bigsqcup (c_i\;Approx(\semfun{a}{q(a)*s})\;h) = \bot$ \\
\ooooo Therefore $(\sqcup c_i)\;Approx(\semfun{a}{q(a)*s})\;h = \bot$ \\
\oooo Therefore $\forall h \in p * s.\; (\sqcup c_i)\; Approx(\semfun{a}{q(a)*s})\;h = \bot$ \\
...
\inferrule*[right=TermPointsTo]
          {\judgeACtx{\Delta} \\
           \judgeWK[\restrictkind{\Delta}]{A}{\bigstar} \\
           \judgeE[\restrictkind{\Delta}]{\restricttype{\Delta}}{e}{\reftype{A}} \\
           \judgeE[\restrictkind{\Delta}]{\restricttype{\Delta}}{e'}{A}}
...
\inferrule*[right=SpecTriple]
          {\judgeA{p}{\assert} \\
           \judgeA{A}{\bigstar} \\
           \judgeC[\restrictkind{\Delta}]{\restricttype{\Delta}}{c}{A} \\
...
Now, we'll consider the interpretation of the syntax. Ultimately,
...
To define this, we'll need to give a mutually-recursive definition
between the interpretation the two judgements $\judgeACtx{\Delta}$,
$\judgeA{\Delta}{\omega}$. 
