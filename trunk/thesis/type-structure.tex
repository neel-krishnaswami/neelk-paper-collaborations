...
The substitution $\theta_\Theta$ can be used to substitute for each of
the free variables in $\tau$ (or $A$), and $\normalize{\tau}$ computes
...
One abbreviation we will make use of is to write $\Run(e)$ for
$\letv{[x]}{e}{x}$. Just as $[c]$ embeds a computation of type $A$ into
an expression term of type $\monad{A}$, the abbreviation $\Run(e)$
does the reverse, giving a computation of type $A$ from an expression
of type $\monad{A}$.
...
mutually recursive interpretation functions,
$\interpE{\judgeE{\Gamma}{e}{A}}$ and
$\interpC{\judgeC{\Gamma}{c}{A}}$, which interpret pure expressions
and computations respectively, are given in
figure~\ref{lang-pure-interp} and figure~\ref{lang-monadic-interp}.

In the types I give for these interpretation, for readability I elide
occurences of $(K,K)$, since they never change and should be obvious
from context. Furthemore, I confuse $n$-ary and iterated products, and 
sometimes suppress contexts (e.g., write $\interp{A}\;\theta$ instead
of $\interp{\judgeWK{A}{\bigstar}}\;\theta$) when it would either be
cluttered or run off the page. 
...
\begin{displaymath}
  \begin{array}{lcl}
    \interpE{\judgeE{\Gamma}{e}{A}} & \in & \Pi\; \theta:\interpset{\Theta}.\; \interp{\Theta \vdash \Gamma}\;\theta\to \interp{\judgeWK{A}{\bigstar}}\;\theta \\[1em] 
    \interpE{\judgeE{x_1:A_1, \ldots, x_n:A_n}{x_i}{A_i}}\;\theta\;\gamma
       & = & \pi_{x_1:A_1, \ldots, x_n:A_n \vdash x_i:A_i}(\gamma)
    \\
   ...
   & = & 
     ...
     \mbox{ by EKeq} \\[2em]
  \mathit{iter}_A[i,f](0) & = & i \\
  \mathit{iter}_A[i,f](n+1) & = & f(\mathit{iter}_A[i,f](n)) \\[1em]
  \pi_{x_i} & = & \mbox{project the component corresponding to $x_i$ in a context $x_1:A_1, \ldots, x:_n:A_n$}\\
...
\begin{displaymath}
  \begin{array}{lcl}
    \interpC{\judgeC{\Gamma}{c}{A}} 
    & \in & \Pi \; \theta:\interpset{\Theta}.\; \interp{\Theta \vdash \Gamma}\;\theta\to T(\interp{\judgeWK{A}{\bigstar}}\;\theta) \\[1em] 
    ...
    & = & 
       \mbox{let }m : \interp{\monad{A}}\theta = \interpE{\judgeE{\Gamma}{e}{\monad{A}}}\;\theta\;\gamma \mbox{ in}\\
...
\label{soundness-of-substitution}
\begin{enumerate}
\item If we know that $\judgeE{\Gamma, y:A, \Gamma'}{e}{B}$ and $\judgeE{\Gamma}{e'}{A}$
  ...
  then \\ $\interpE{\judgeE{\Gamma}{[e'/y]e}{B}}\;\theta\;(\gamma,\gamma') = 
        \interpE{\judgeE{\Gamma, y:A, \Gamma'}{e}{B}}\;\theta 
                \;\left(\gamma, 
                        \interpE{\judgeE{\Gamma}{e'}{A}}\;\theta\;\gamma,
                        \gamma'
                \right)$
...
\item If we know that $\judgeC{\Gamma, y:A, \Gamma'}{c}{B}$ and $\judgeE{\Gamma}{e'}{A}$
  ...
  then \\ $\interpC{\judgeE{\Gamma}{[e'/y]c}{B}}\;\theta\;(\gamma,\gamma') = 
        \interpC{\judgeC{\Gamma, y:A, \Gamma'}{c}{B}}\;\theta \;
                \left(\gamma,
...
\label{soundness-of-equality-rules}
We have that:
\begin{enumerate}
...
\begin{lemma*}{(\ref{soundness-of-weakening}, page \pageref{soundness-of-weakening}: Soundness of Weakening)}
  Suppose $\judgeE[\Theta]{\Gamma}{e}{A}$. 
  \begin{enumerate}
   \item It is the case that 
    $\interpE{\judgeE[\Theta, \alpha:\kappa]{\Gamma}{e}{A}}\;(\theta, \tau)\;\gamma$ is equal to \\
...
\begin{lemma*}{(\ref{soundness-of-type-substitution}, page \pageref{soundness-of-type-substitution}: Soundness of Type Substitution)}
If we know that $\judgeWK{\tau}{\kappa}$, then 
\begin{itemize}
\item If $\judgeE[\Theta, \alpha:\kappa]{\Gamma}{e}{A}$, then 
       $\interpE{\judgeE[\Theta, \alpha:\kappa, \Theta']{\Gamma}{e}{A}}\;(\theta,\interpset{\tau}\;\theta, \theta')\;\gamma$ is
       equal to \\ $\interpE{\judgeE{[\tau/\alpha]\Gamma}{[\tau/\alpha]e}{[\tau/\alpha]A}}\;\theta\;\gamma$
\item If $\judgeC[\Theta, \alpha:\kappa]{\Gamma}{c}{A}$, then 
       $\interpC{\judgeC[\Theta, \alpha:\kappa]{\Gamma}{c}{A}}\;(\theta,\interpset{\tau}\;\theta, \theta')\;\gamma$ 
       is equal to \\ $\interpC{\judgeC{[\tau/\alpha]\Gamma}{[\tau/\alpha]c}{[\tau/\alpha]A}}\;\theta\;\gamma$
\end{itemize}
\end{lemma*}
...
\item case EHyp: 
  ...
  \begin{eqnproof}
        [\interpE{\judgeE[\Theta, \alpha:\kappa, \Theta']
                         {\Gamma}
                         {x_i}{A_i}}\;(\theta, \interpset{\tau}(\theta), \theta')\;\gamma]
    \eline{\pi_{x_i}(\gamma)}
          {Semantics}
  \end{eqnproof}

\noindent Now, observe that the tuple $\gamma$ is an element of $\interp{\Theta,\alpha:\kappa,\Theta' \vdash \Gamma}\;(\theta, \interpset{\tau}\;\theta, \theta')$.

  \begin{eqnproof}[\interp{\Theta,\alpha:\kappa \vdash \Gamma}\;(\theta, \interpset{\tau}\;\theta) =]
     \eline{\interp{\interp{\judgeWK[\Theta,\alpha:\kappa, \Theta']{A_1}{\bigstar}}\;
                       (\theta, \interpset{\tau}\;\theta, \theta') 
                    \times \ldots \times 
                    \interp{\judgeWK[\Theta,\alpha:\kappa]{A_n}{\bigstar}}\;
                       (\theta, \interpset{\tau}\;\theta, \theta')}}
           {}
     \eline{\interp{\judgeWK{[\tau/\alpha]A_1}{\bigstar}}\;(\theta, \theta')
            \times \ldots \times 
            \interp{\judgeWK{[\tau/\alpha]A_n}{\bigstar}}\;(\theta, \theta')}
           {}
     \eline{\interp{\Theta \vdash [\tau/\alpha]\Gamma}\;(\theta,\theta')}
           {}
  \end{eqnproof}

So $\gamma$ is also an element of $\interp{\Theta,\Theta' \vdash [\tau/\alpha]\Gamma}\;(\theta,\theta')$ as well, and so 

  \begin{eqnproof}[\interpE{\judgeE{[\tau/\alpha]\Gamma}{x_i}{[\tau/\alpha]A_i}}\;\theta\;\gamma=]
    \eline{\pi_{x_i}(\gamma)}
          {Semantics}
    \eline{\interpE{\judgeE[\Theta, \alpha:\kappa, \Theta']
                         {\Gamma}
                         {x_i}{A_i}}\;(\theta, \interpset{\theta}\;\tau, \theta')\;\gamma}
          {}
  \end{eqnproof}
...
\begin{lemma*}{(\ref{soundness-of-substitution}, page \pageref{soundness-of-substitution}: Soundness of Substitution)}
\begin{enumerate}
\item If we know that $\judgeE{\Gamma, y:A, \Gamma'}{e}{B}$ and $\judgeE{\Gamma}{e'}{A}$
  and $\Theta \vdash \theta$, 
  then \\ $\interpE{\judgeE{\Gamma}{[e'/y]e}{B}}\;\theta\;(\gamma,\gamma') = 
   ...
   $\interpE{\judgeE{\Gamma, \Gamma'}{[e'/y]x_i}{B}}\;\theta\;(\gamma, \gamma')$
    ...
    \begin{eqnproof}
      \eline{\interpE{\judgeE{\Gamma, \Gamma'}{x_i}{B}}\;\theta\;(\gamma, \gamma')}
            { Subst.}
      \eline{\pi_{x_i}(\gamma, \gamma')}
            { Semantics}
      \eline{\pi_{x_i}(\gamma, \interpE{\judgeE{\Gamma}{e'}{A}}\;\theta\;\gamma, \gamma')}
            { Adjusting $i$ }
...
\begin{lemma*}{(\ref{soundness-of-equality-rules}, page \pageref{soundness-of-equality-rules}: Soundness of Equality Rules)}
We have that:
\begin{enumerate}
\item If $\judgeEq{\Gamma}{e}{e'}{A}$, then $\judgeE{\Gamma}{e}{A}$ and 
$\judgeE{\Gamma}{e'}{A}$ and 
...
\item case EqPairFst: From $\judgeEq{\Gamma}{\fst{\pair{e_1}{e_2}}}{e_1}{A_1}$, we have:
  ...
  \begin{eqnproof}
    \eclaim[1]{\judgeEq{\Gamma}{\fst{\pair{e_1}{e_2}}}{e_2}{A_1}}
              {Hypothesis}
...
\item case EqFunEta: 
  ...
  \begin{eqnproof}
    ...
    \eclaim[2]{\judgeEq{\Gamma, x:A}{e\;x}{e'\;x}{B}}
              {Inversion}
    \eclaim[3]{\judgeE{\Gamma,x:A}{e\;x}{B}}
              {Induction}
    \eclaim[4]{\judgeE{\Gamma,x:A}{e'\;}{B}}
              {Induction}
    \eclaim[5]{\judgeE{\Gamma}{e}{A \to B}}
              {Inversion and $x\not \in FV(e)$}
    \eclaim[6]{\judgeE{\Gamma}{e'}{A \to B}}
              {Inversion and $x\not \in FV(e')$}
  \end{eqnproof}
  \begin{eqnproof}[\mbox{For arbitrary }v,]
    \eline[\interpE{\judgeE{\Gamma,x:A}{e'\;x}{B}}\;\theta\;(\gamma,v)]
          {\interpE{\judgeE{\Gamma,x:A}{e\;x}{B}}\;\theta\;(\gamma,v)}
          {Induction}
    \eline[\interpE{\judgeE{\Gamma,x:A}{e}{A \to B}}\;\theta\;(\gamma,v)\;v]
          {\interpE{\judgeE{\Gamma}{e}{A \to B}}\;\theta\;\gamma}
          {Semantics, $x\not \in FV(e)$}
    \eline[\interpE{\judgeE{\Gamma,x:A}{e'}{A \to B}}\;\theta\;(\gamma,v)\;v]
          {\interpE{\judgeE{\Gamma}{e'}{A \to B}}\;\theta\;\gamma}
          {Semantics, $x\not \in FV(e')$}
    \eline[\interpE{\judgeE{\Gamma}{e'}{A \to B}}\;\theta\;\gamma]
          {\interpE{\judgeE{\Gamma}{e}{A \to B}}\;\theta\;\gamma}
          {Transitivity}
  \end{eqnproof}
