...
However, before we can give a proper proof of this theorem, we need
to specify and prove correct the $\ctext{mark\_unready(c)}$ procedure,
...
\oooo $\letv{\_}{[a.reads := \emptyset]}{}$ \\
\oooo $a.obs := \emptyset]$ \\
\oooo These commands ensure that $\mathit{heap}(D, h'')$ is satisfied. 
\ooo Now, by appeal to a 
...
To simplify matters, we'll split into two cases, one for when the
argument is already unready, and one for when it is ready.
...
\forall \psi.\; \mspec{G(\psi)}
                     {\return v}
                     {a}{G(\psi) \land \exists z.\; a = (v, z) \land \mathit{set}(z, \emptyset, \emptyset)} 
\\[1em]

\mbox{\textsc{ABind}} & 
\forall \psi.\; \mspec{G(\phi \otimes \psi)}
                     {e}
                     {a}{G(\phi' \otimes R(u', \psi)) \land \exists z.\;a = (v', r') \land \mathit{set}(z, r', r')} 
                \specand \\
&  \forall \psi.\; \mspec{G(\phi' \otimes \psi)}
                        {f\;v'}
                        {a}{G(\phi'' \otimes R(u'', \psi)) \land \exists z.\; a = (v'', z) \land \mathit{set}(z, r'', r'')}\\
&  \specimp \\
&  \forall \psi.\; \mspec{G(\phi \otimes \psi)}
                        {\bind e\;f}
                        {a}{a = (v'', r' \cup r'') \land G(\phi'' \otimes R(u' \cup u'', \psi))} 
\\[1em]
\mbox{\textsc{AReady}} & 
  \setof{\ready{\phi}{c}{v}} \specimp \\
& \forall \psi.\; \mspec{G(\phi \otimes \psi)}
                       {\readcell c}
                       {a}{G(\phi \otimes \psi) \land \exists z.\;a = (v, z) \land \mathit{set}(\, \setof{c}, \setof{c})} 
\\[1em]
\mbox{\textsc{AUnready}} & 
                 ...
                 & \left<a.\; 
                     \begin{array}{l}
                       G(R(\setof{c}, \phi') \otimes \cellpos{c}{e}{v}{r} \otimes R(u \cup \setof{c}, \psi)) 
                        \;\land \\
                        \exists z.\;a = (v, z) \land \mathit{set}(z, \setof{c}, \setof{c})
                     \end{array}\right> 
  \end{array}
\\[3em]
...
& \begin{array}{ll}
    ...
    &  \left<a.\; G(\localref{r}{v} \otimes \psi) 
                  \land  \exists z.\;a = (v,z) \land \mathit{set}(z, \emptyset, \emptyset)
       \right> \\
     \end{array}
\\[2em]
...
\\[2em]
\mbox{\textsc{ANewCell}}
& \forall \psi.\; \mspec{G(\psi)}{\newcell e}{(c,z)}{G(\cellneg{c}{e} \otimes \psi) \land \mathit{set}(z, \emptyset, \emptyset)}
\end{array}
