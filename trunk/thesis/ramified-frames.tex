...
In Figure~\ref{closedness}, we define the $\mathit{closed}(\phi, s)$ predicate,
...
% \oo Now, from the fact $\mathit{Transduce}(vs, i, \phi, m, f\;vs)$ holds, we know that \\
% \oo $\unready{\theta \otimes \phi_n}{m}$ and $\mathit{closed}(\phi_n, \domain{\phi_n} \cup \setof{i})$ \\
% \oo and $\ready{\theta' \otimes \phi_{n+1}}{(f\; vs)_n}$ and $\mathit{closed}(\phi_{n+1}, \domain{\phi_{n+1}} \cup \setof{i})$ \\
% \oo and \\
% 
% \oo We want to show $\unready{\theta \otimes \phi_n \otimes \sigma_n}{o}$ \\
% \oox and $\mathit{closed}(\phi_n \otimes \sigma_n, \domain{\phi_n \otimes \sigma_n} \cup \setof{o})$ \\
% \oox and $\ready{\theta' \otimes \phi_{n+1} \otimes \sigma_{n+1}}{o}{(g\;(f\;vs))_n}$ \\
% \oox and $\mathit{closed}(\phi_{n+1} \otimes \sigma_{n+1}, \domain{\phi_{n+1} \otimes \sigma_{n+1}} \cup \setof{o})$ \\
% \oox and $\forall \psi.\; \mspec{G(\theta \otimes \phi_{n} \otimes \sigma_{n} \otimes \psi)}
%                                 {\readcell o}
%                                 {a}{\exists u'.\;a = ((g\;(f\;vs))_n, \setof{o}) \land 
%                                         u' \subseteq \domain{\theta' \otimes \phi_{n+1} \otimes \sigma_{n+1}} \land 
%                                         G(\theta' \otimes \phi_{n+1} \otimes \sigma_{n+1} \otimes R(u',\psi))}$\\
% \ooo Now instantiate the quantifiers of $\mathit{Transduce}(f\;vs, m, \theta, o, g(f\;vs))$ with \\
% \ooo $n = n$, $\theta = \theta \otimes \sigma_n$, $\theta' = \theta' \otimes \sigma_{n+1}$ \\
% 
% assume we have $\psi$ and are in a prestate $G(\theta \otimes \phi_n \otimes \psi)$ \\
% \ooo Since $\unready{\theta}{i}$, it follows that $\unready{\theta \otimes \phi_n}{o}$ \\
% \ooo Hence $G(\theta \otimes \cellneg{c}{e} \otimes \psi)$ \\
% \ooo $\readcell o$ \\
% \ooo By the \textsc{AUnready} rule, we end in a state \\
% \ooo $G(\theta' \otimes \cellpos{o}{e}{f\;vs_n}{\setof{i}} \otimes R(u \cup \setof{o}, \psi))$ \\
% \ooo Note that since $\ready{\theta'}{i}{vs_n}$, we have $\ready{\theta' \otimes \phi_{n+1}}{o}{(f\;vs)_n}$ \\
\end{tabbedproof}
\end{proof}
