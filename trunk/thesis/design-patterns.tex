...
\nextline
$\iterpred_\tau(\One\;i, xs, S) \qquad\qquad$\>$\equiv \bot$
\nextline
...
Since $c$ is an interior pointer, there may be more collection state
occurring before $c$, though we don't care about the details of that 
state --- this is why $\top$ is conjoined to the list predicate. However,
the collection state does depend on the whole list, which is why $P$ 
is conjoined with the state $\top * \listpred(\tau, c, xs)$. 

Adding the magic wand to the formula , so that we get $P \wand (P \land (\top
* \listpred(\tau, c, xs)))$, intuitively means ``if you give me the
...
On the line after that, we give the case for $\iterpred_\tau(\Merge(\tau_1, \tau_2, f, i_1, i_2), xs, S)$. 
...
\begin{displaymath}
  ...
  \begin{array}{l}
    \setof{\iterpred_\alpha(i, xs, S) * \iterpred_\beta(i',xs',S') \land S \cap S' = \emptyset} \\
    \mergeiter_{\alpha\;\beta\;\gamma}(f, i, i') \\
...
\begin{specification}
\nextline $\setof{\iterpred_\alpha(i, xs, S) * \iterpred_\beta(i',xs',S') \land S \cap S' = \emptyset}$ 
\nextline $\Merge(\alpha, \beta, f, i, i')$
\nextline $\setof{\iterpred_\alpha(i, xs, S) * \iterpred_\beta(i',xs',S') \land S \cap S' = \emptyset \land a = \Merge(\alpha, \beta, f, i, i')}$ 
\nextline $\{$\=$\iterpred_\alpha(i, xs, S) * \iterpred_\beta(i',xs',S') \land S \cup S' = S \uplus S' \land \mergemath\;f\;xs\;xs' = \mergemath\;f\;xs\;xs'$
