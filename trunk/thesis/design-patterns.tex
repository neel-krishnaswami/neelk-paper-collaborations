...
$\listpred(\tau, c, \epsilon)$ \qquad\= $\equiv$ \= $c \pointsto_\tau \Nil$ 
\nextline
$\listpred(\tau, c, x\cdot xs)$ \> $\equiv$ \> 
   $\exists c'.\; c \pointsto_\tau \Cons(\tau, x, c') * list(c', xs)$ 
...
\nextline $\setof{\exists c'. i' \pointsto c' * P \land \listpred(\alpha, c, ys) \land (\top * \listpred(\alpha, c', ys)) \land \exact{P}}$ 
\nextline $\letv{c'}{\comp{!i'}}{}$ 
\nextline $\setof{i' \pointsto c' * P \land \listpred(\alpha, c, ys) \land (\top * \listpred(\alpha, c', ys)) \land \exact{P}}$ 
\nextline $\setof{i' \pointsto c' * P \land (\exists p.\; c' \pointsto p * \listcontentpred(\alpha, p, ys)) \land (\top * \listpred(\alpha, c', ys)) \land \exact{P}}$ 
\nextline $\letv{p}{\comp{!c'}}{}$ 
\nextline $\setof{i' \pointsto c' * P \land (c' \pointsto p * \listcontentpred(\alpha, p, ys)) \land (\top * \listpred(\alpha, c', ys)) \land \exact{P}}$ 
\nextline $\Run\;$\=$\Listcase(p,$ 
\nextline \> $\Nil $\=$\to$ 
\nextline \> \> Framing $\exact{P}$
\nextline \> \> $\setof{p = \Nil \land i' \pointsto c' * P \land (c' \pointsto p * \listcontentpred(\alpha, p, ys)) \land (\top * \listpred(\alpha, c', ys)) }$ 
\nextline \> \> $\setof{p = \Nil \land i' \pointsto c' * P \land (c' \pointsto \Nil \land ys = \epsilon) \land (\top * \listpred(\alpha, c', ys)) }$ 
\nextline \> \> $\comp{\None}$ 
\nextline \> \> $\setof{a = \None \land p = \Nil \land i' \pointsto c' * P \land (c' \pointsto \Nil \land ys = \epsilon) \land (\top * \listpred(\alpha, c', ys)) }$ 
\nextline \> \> Restoring frame $\exact{P}$ 
\nextline \> \> $\setof{a = \None \land xs = \epsilon \land i' \pointsto c' * P \land \listpred(\alpha, c, ys) \land (\top * \listpred(\alpha, c', ys)) \land \exact{P}}$ 
\nextline \> \> $\setof{a = \None \land xs = \epsilon \land \iterpred_\alpha(i, xs, \setof{P}) * colls(C, \setof{P})}$
\nextline \> $\Cons(y, c) \to$ 
\nextline \> \> Framing $\exact{P}$
\nextline \> \> $\setof{p = \Cons(y, c) \land i' \pointsto c' * P \land (c' \pointsto p * \listcontentpred(\alpha, p, ys)) \land (\top * \listpred(\alpha, c', ys))}$ 
\nextline \> \> $\setof{p = \Cons(y, c) \land i' \pointsto c' * P \land (c' \pointsto  * \listcontentpred(\alpha, p, ys)) \land (\top * \listpred(\alpha, c', ys))}$ 
\end{specification}
\end{itemize}

Note that this is not a structural induction on $i$; the $\Filter(p,i)$ case
has a non-structural recursive call. 
\end{proof}
